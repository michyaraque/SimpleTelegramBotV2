...
class Client {
     ...
     * @return object
     */
    public static function request(string $method, array $params = null): ?object {
        $url = env('TELEGRAM_ENDPOINT') . self::$api_key . "/" . $method . "?" . http_build_query($params); 
        /* $url = env('TELEGRAM_ENDPOINT') . self::$api_key . "/" . $method; 
        ...
        $output_from_request = $request->getBody(); */
        ...
        if(env('TELEGRAM_LOG_TRACE') && property_exists($output_decode, 'ok') && $output_decode->ok == false) {
    ...
    public static function editMessageText(string $chat_id, int $message_id, string $text, array $parameters = []): object {
        ...
        if(!empty($parameters['reply_markup']) && !empty(Updates::callbackId())) {
